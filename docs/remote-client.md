...
1. 打开 **Tizhi AI Desktop**。默认网页登录地址为 `https://gr.gr-iot.cn:3081`，目录连接地址为 `wss://gr.gr-iot.cn:3082`。
...
旧版客户端保留原服务器设置。升级后在 **本机目录** 页面将网页登录地址改为 `https://gr.gr-iot.cn:3081`、目录连接地址改为 `wss://gr.gr-iot.cn:3082`，重新登录并选择该服务器下需要接入的本机目录。服务器地址参与目录授权隔离。
