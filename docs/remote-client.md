...
4. 连接成功后点击 **返回会话**，在 30 的工作区列表选择对应本机工作区；Agent 的本机文件工具可以读取、写入、编辑和搜索该目录。
...
此版本仅授权所选目录中的文件操作，未授权本机 Shell 或 Office 自动化。Agent 在 30 个人工作区执行终端命令的现有功能仍然保留；关闭桌面端不会停止 30 上托管的测试服务。
...
每次远程文件操作重新验证当前账号，并在独立的本机进程执行；请求限于授权目录的相对路径，拒绝目录外的符号链接。退出账号会取消尚未完成的文件操作，单次文件进程最多运行 30 秒。该功能是文件工具的目录限制，不是通用操作系统沙箱。
