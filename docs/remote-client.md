...
已保存的目录在连接前或握手后的账号验证临时失败时会自动重试，每次重试仍验证当前登录账号。恢复连接无需重启应用；退出账号、断开目录或关闭客户端会取消重试。服务器明确拒绝目录凭据时仍需重新授权。
...
桌面端 0.1.2 配合新版 dsh-passwords 和 dsh-better-sidebar，支持在对应本机工作区通过 `@` 搜索目录内文件，右侧文件面板可展开目录、搜索、预览文本、图片、PDF 与 HTML。读取走本机独立文件进程，不依赖 Shell。更新后彻底退出旧客户端并重新打开，原账号已授权目录会恢复连接。

文件列表默认最多 5000 条，依赖与构建目录默认不参与 @ 索引；管理员可通过服务端配置调整。符号链接不在列表中展示，预览不能访问授权目录外的目标。断线、切换账号和撤销授权都会停止本机文件访问。文件面板目前提供本机读取，修改继续通过 Agent 文件工具执行。
